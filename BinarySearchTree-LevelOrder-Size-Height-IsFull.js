...
*/
toArrLevelorder(current = this.root, treeArr = []){
    if(current === null){
        return null;
    }
    if(current == this.root){
        treeArr.push(current.data);
    }
    while(current){
        if(current.left !== null){
            treeArr.push(current.left.data);
            this.toArrLevelorder(current.left, treeArr);
        }
        if(current.right !== null){
            treeArr.push(current.right.data);
            this.toArrLevelorder(current.right, treeArr);
        }
        return treeArr;
    }
}
...
// Using a queue
toArrLevelorder2(current = this.root){
    const queue = [];
    const vals = [];
    ...
    while(queue.length > 0){
        const dqueNode = queue.shift();
        vals.push(dqueNode.data);

        if(dqueNode.left){
            queue.push(dqueNode.left);
        }
        if(dqueNode.right){
            queue.push(dqueNode.right);
        }
    }
    return vals;
}
 ...
 */
size(node = this.root, treeArr = []){
    this.toArrLevelorder(node, treeArr);
    return treeArr.length;
}
...
console.log(fullTree.toArrLevelorder2());
